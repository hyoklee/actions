...
def repo_url(url):
    """Normalise a clone URL to the browsable https form, no trailing .git.

    sources.json carries whatever the driver cloned from or read out of
    remote.origin.url, which may be an ssh remote on a developer's machine.
    """
    url = (url or "").strip()
...
def repo_slug(url):
    """owner/name from a repo URL, for the link text. Whole URL if it does not
    look like one -- better an ugly cell than a wrong name."""
    m = re.search(r"([^/]+/[^/]+)$", repo_url(url))
...
def render(args, data, sources, build_note):
    ...
    a("")
    a("| source | repo | ref | commit |")
    a("| --- | --- | --- | --- |")
    for label, key, up_repo, up_ref in (
        ("HDF5",      "hdf5",   "https://github.com/HDFGroup/hdf5",    "develop"),
        ("netCDF-C",  "netcdf", "https://github.com/Unidata/netcdf-c", "main"),
        ("clio-core", "clio",   "https://github.com/iowarp/clio-core", "dev"),
    ):
        sha = sources.get(f"{key}_sha", "?")
        repo = repo_url(sources.get(f"{key}_repo") or up_repo)
        ref = sources.get(f"{key}_ref") or up_ref
        slug = repo_slug(repo)
        commit = f"[`{sha[:12]}`]({repo}/commit/{sha})" if len(sha) >= 12 else f"`{sha[:12]}`"
        a(f"| {label} | [{slug}]({repo}) | `{ref}` | {commit} |")
    a("")
...
explains why the baseline row was not green — `nc_perf` did not compile with
MSVC, which failed 4 of the 204 tests in *every* variant until the branch under
test picked up Unidata/netcdf-c#3446.

## What is being tested

The forks, not the upstreams: `hyoklee/hdf5@develop`, `hyoklee/netcdf-c@main`
and `hyoklee/core@dev` (the clio-core fork is named `core`). Each is kept a
strict superset of its upstream, so every run is upstream plus the fixes in
flight. The **repo** column of each platform page names and links the exact
repository and commit that run built — read it rather than assuming.

## How the runs are triggered

Daily on a cron (Linux 13:00, macOS 15:00, Windows 17:00 UTC), but only when
one of those three branches has moved since the last run — the last tested
triple is stamped into each platform page as an HTML comment. Run any of them
by hand from the Actions tab with **force** to override the gate.
